...
class dataAuth {
	...
	public function update($db, $username, $lastname, $firstname, $email, $password, $jour, $mois, $annee, $sex, $ville, $departement, $region, $pays){
		$user = $this->user();
		if($password != "") {
			$password = $this->hashPassword($password);
			$db->query("UPDATE users SET username = ?, lastname = ?, firstname = ?, email = ?, date = ?, sex = ?, pays = ?, departement = ?, region = ?, ville = ?, password = ?, access = 1 WHERE id = $user->id", [ 
				$username, 
				$lastname,
				$firstname,
				$email,
			...
			$db->query("UPDATE users SET username = ?, lastname = ?, firstname = ?, email = ?, date = ?, sex = ?, pays = ?, departement = ?, region = ?, ville = ?, access = 1 WHERE id = $user->id", [ 
				$username, 
				$lastname,
				$firstname,
				$email,
	...
	public function register($db, $username, $lastname, $firstname, $email, $password, $jour, $mois, $annee, $sex, $ville, $departement, $region, $pays){
		$password = $this->hashPassword($password);
		$token = Str::random(60);
		$db->query("INSERT INTO users SET username = ?, lastname = ?, firstname = ?, email = ?, date = ?, sex = ?, pays = ?, departement = ?, region = ?, ville = ?, password = ?, confirmation_token = ?, access = 1", [ 
			$username, 
			$lastname,
			$firstname,
			$email,
		...
		$message = Swift_Message::newInstance('confirmation de compte')
			->setFrom(['your_mail' => 'mail.com'])
			->setTo(["$to"])
			->setBody("http://localhost/depot/PHP_my_meetic\r\n/confirm.php?id=$user_id\r\n&token=$token");
	...
	public function restrict(){
		if(!$this->session->read('auth') || !($this->session->read('auth')->active == 1)) {
	...
	public function checkReset($db, $user_id, $token) {
		return $db->query('SELECT * FROM users WHERE id = ? AND reset_token = ? AND reset_at > DATE_SUB(NOW(), INTERVAL 30 MINUTE) AND active = 1', [$user_id, $token])->fetch();
	}
}